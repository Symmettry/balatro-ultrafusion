SMODS.current_mod.menu_cards = function()
    -- </3
    local count = 0
    for _ in pairs(G.P_CENTERS) do
        count = count + 1
    end
    local BASELINE = 380
    UF.INCREASE_WEIGHT = (count - BASELINE) / 30 + 10
    ...
    local function get_joker_display_name(key)
    ...
    local function generate_dot(fusions)
        ...
        for tier = 1, max_tier do
            ...
            for _, node in ipairs(nodes_by_tier[tier]) do
                local label = get_joker_display_name(node)
                lines[#lines + 1] = "        " .. q(node) .. " [label=" .. q(label) .. ", fillcolor=" .. q(fill) .. "];"
            end
